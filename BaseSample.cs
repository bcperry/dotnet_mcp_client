...
internal abstract class BaseSample
{
    ...
    protected static Kernel CreateKernelWithChatCompletionService()
    {
        ...
        string endpoint = config["OpenAI:Endpoint"] ?? "https://{your-custom-endpoint}.openai.azure.com/";
        string deploymentName = config["OpenAI:DeploymentName"] ?? "gpt-4o-mini";

        Console.WriteLine($"Using OpenAI endpoint: {endpoint}");
        // Create kernel
